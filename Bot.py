...
def resolver_captcha(tipo, sitekey, page_url):
    ...
    try:
        if tipo == "hcaptcha":
            data = {"key": CAPTCHA_API_KEY, "method": "hcaptcha",
                    "sitekey": sitekey, "pageurl": page_url, "json": 1}
        elif tipo == "turnstile":
            data = {"key": CAPTCHA_API_KEY, "method": "turnstile",
                    "sitekey": sitekey, "pageurl": page_url, "json": 1}
        else:
            data = {"key": CAPTCHA_API_KEY, "method": "userrecaptcha",
...
def injetar_e_submeter(page, tipo, token):
    try:
        if tipo == "hcaptcha":
            page.evaluate(f"""
                ...
                // Tenta chamar callback do hcaptcha
                if (typeof hcaptcha !== 'undefined') {{
                    const widgetId = Object.keys(hcaptcha).find(k => hcaptcha[k] && hcaptcha[k].execute);
                }}
            ...
            page.evaluate(f"""
                ...
                if (form) form.submit();
            """)
        log.info("Token injetado e form submetido.")
        time.sleep(8)
        log.info("URL apos submit: %s", page.url)
    except Exception as e:
        log.warning("Erro ao injetar/submeter: %s", e)
...
def tentar_agendar():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Resolve captcha do Radware
            tipo, sitekey = detectar_captcha(page)
            log.info("Captcha: tipo=%s sitekey=%s", tipo, sitekey)
            if tipo and sitekey:
                token = resolver_captcha(tipo, sitekey, page.url)
                if token:
                    injetar_e_submeter(page, tipo, token)
                else:
                    log.error("Captcha nao resolvido.")
                    return

            # Aguarda campo username no Keycloak
            ...
            # Resolve captcha na pagina de senha se necessario
            tipo, sitekey = detectar_captcha(page)
            if tipo and sitekey:
                token = resolver_captcha(tipo, sitekey, page.url)
                if token:
